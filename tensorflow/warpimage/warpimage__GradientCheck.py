...
try:       
    import scipy as sp       
    I1col = sp.misc.face().astype(np.float64)
    if I1col.max() > 1.0:
        I1col /= 255.0    
except:
...
Coords3D_rand = np.random.random( [1,wy,wx])*0.4

C_np_old = np.meshgrid(np.arange(0,wx)  + 2.5 , np.arange(0,wy)-2.5)
Coords3D_old = np.stack((C_np_old[0],C_np_old[1]),axis=0) # 2HW
Coords3D_old += Coords3D_rand
Coords4D_old = np.stack((Coords3D_old,)*N,axis=0) # N2HW

C_np = [  np.zeros([wy,wx])+ 2.5 ,  np.zeros([wy,wx]) -2.5 ]
# Watch out if 
#    assert False
...
Coords3D = np.stack((C_np[0],C_np[1]),axis=0) # 2HW
Coords3D += Coords3D_rand
...
#    for idd ,dev in enumerate( ["/cpu:0","/gpu:0","/cpu:0","/gpu:0","/cpu:0","/gpu:0","/cpu:0","/gpu:0"]):    
    with tf.device(dev):
        ...
        I_test =  I1_batch4D.copy().astype(NPDTYPE)
        C_test = Coords4D.copy().astype(NPDTYPE)
        ...
        y_tf =  my_warpimage(*x_lst,interp_type="BILINEAR") #"BILINEAR","BICUBIC_2POINTS","BICUBIC_4POINTS"
#        y_tf =  my_warpimage(*x_lst,interp_type="BICUBIC_2POINTS") #"BILINEAR","BICUBIC_2POINTS","BICUBIC_4POINTS"
            ...
            if True:
                if N<=1:
                    ...
                    for idc in range(I1col.shape[2]):
                        I1 = I1col[:,:,idc]
                        Coords_sp = np.array([Coords4D_old[0,1,:,:].flatten(),Coords4D_old[0,0,:,:].flatten()])            
                        Iwres_spNDI = (ndimage.map_coordinates(I1,Coords_sp,mode="constant",order=1) )#.reshape(I2.shape)
...
#    
#fig10,axes = plt.subplots(num=20, clear = True, ncols=2,nrows=1)
#axes[0].imshow( I1col)
#axes[0].set_title("Orig")
#axes[1].imshow(y_res_np )
#axes[1].set_title("warped")

#myplt(10+idd , analytic=I1col, num=y_res_np,  title="Original Image + Warped both TF "+ dev )
#exit()
#        
#        plt.pause(3);
