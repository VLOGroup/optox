...
//#include "tensorflow/contrib/icg/ops/warpimage.h"
#include "warpimage.h"
...
template <typename T>
__global__ void WarpimageGPUBicubic(const typename Tensor4<T>::ConstTensor  img ,
  ...
  if ( (idn<wdn) && (idc<wdc) && (idy<wdy) && (idx<wdx) ){      
    ...
    const T fu = floor(float(u)); // dummy so that it works also for integers...
    const T fv = floor(float(v)); 
    const T du = u - fu;
    const T dv = v - fv; 
    ...
    for (int dy = neglim; dy <= poslim; ++dy)
    {
      ...
      if ((c_idy >= 0) && (c_idy < wdy)){
        ...
        buff_y[1+dy] = interpolate_cubic<T>( buff_vals , du, difference2Point);
      }
      else
        buff_y[1+dy] = 0;
    }

    T c_out = interpolate_cubic<T>(buff_y, dv, difference2Point);
...
template <typename T>
__global__ void WarpimageGPU(const typename Tensor4<T>::ConstTensor  img ,
                  const typename Tensor4<T>::ConstTensor flow,
                  typename Tensor4<T>::Tensor out, int interp_type){  
...
template <typename T>
struct WarpimageFunctor<GPUDevice, T> {
  void operator()(tensorflow::OpKernelContext* context, 
    ...
    // printf("flow (N2HW): %ld, %ld, %ld, %ld\n", flow.dimensions()[0],flow.dimensions()[1] ,flow.dimensions()[2], flow.dimensions()[3]);
    ...
    // WarpimageGPU<T><<<grid_size,block_size>>>(img ,flow, out,  interp_type);
    if (interp_type == tficg::INTERP_TYPE_BILINEAR)
      WarpimageGPU<T><<<grid_size,block_size,0,d.stream()>>>(img ,flow, out,  interp_type);
    else if (interp_type == tficg::INTERP_TYPE_BICUBIC_2POINTS or interp_type == tficg::INTERP_TYPE_BICUBIC_4POINTS)
      WarpimageGPUBicubic<T><<<grid_size,block_size,0,d.stream()>>>(img ,flow, out,  interp_type);
    else
...
// template struct WarpimageFunctor<GPUDevice, float>
// template struct WarpimageFunctor<GPUDevice, int>

#define REGISTER_GPU_FUNCTOR(T) \
template struct WarpimageFunctor<GPUDevice, T>;
...
// template <typename T>
// __global__ void Warpimage3DGPU(const typename tensorflow::TTypes<T,5>::ConstTensor  img ,
//                   const typename tensorflow::TTypes<T,5>::ConstTensor flow,
...
//   // flow (idn,0..2,idy,idx,idz)        // N3HWD 0=y,1=x,2=z
//   // img (idn,idy,idx,idc,idz)            // NHWCD
//     const T x = flow(idn,0, idy, idx,idz);
//     const T y = flow(idn,1, idy, idx,idz);
//     T z = flow(idn,2, idy, idx,idz);
...
// template <typename T>
// struct Warpimage3DFunctor<GPUDevice, T> {
//   void operator()(tensorflow::OpKernelContext* context, 
//                   const typename tensorflow::TTypes<T,5>::ConstTensor  &img ,
//                   const typename tensorflow::TTypes<T,5>::ConstTensor &flow,
//                   typename tensorflow::TTypes<T,5>::Tensor &out)
...
//     Warpimage3DGPU<T><<<grid_size,block_size>>>(img ,flow, out,  true);
//   }
// };

// #define REGISTER_GPU_FUNCTOR(T) \
// template struct Warpimage3DFunctor<GPUDevice, T>;
// // for a full list of type macros see https://github.com/tensorflow/tensorflow/blob/master/tensorflow/core/framework/register_types.h
// TF_CALL_GPU_NUMBER_TYPES_NO_HALF(REGISTER_GPU_FUNCTOR);
// // TF_CALL_INTEGRAL_TYPES(REGISTER_GPU_FUNCTOR);
// #undef REGISTER_GPU_FUNCTOR
...
template <typename T>
__global__ void WarpimageGradientsGPUBicubic(
                  const typename Tensor4<T>::ConstTensor  img ,
                  const typename Tensor4<T>::ConstTensor  flow ,
  ...
  if ( (idn<wdn) && (idc<wdc) && (idy<wdy) && (idx<wdx) ){
    ...
    backpolate_cubic<T>(buff_y, currGrad, dv ,difference2Point);
    ...
    for (int dy = neglim; dy <= poslim; ++dy)
    {
      ...
      if ((c_idy >= 0) && (c_idy < wdy)){
        ...
        backpolate_cubic<T>(buff_vals, currGrad, du ,difference2Point);
...
template <typename T>
__global__ void WarpimageGradientsGPU(
                  const typename Tensor4<T>::ConstTensor  img ,
                  const typename Tensor4<T>::ConstTensor  flow ,
                  const typename Tensor4<T>::ConstTensor  gradIn ,
                  typename Tensor4<T>::Tensor gradImg,
    ...
    //   out(idn,idy,idx,idc) = 0;
      ...
      if ((y1 >= 0) && (y1 < wdy) && (x1 >= 0) && (x1 < wdx))
        ...
        //tensorflow::CudaAtomicAdd
         ...
         atomicAdd( &gradImg(idn,y1,x1,idc)  ,  (T) (gIn * (1.0 - dv) * (1.0 - du) ));

      if ((y1 >= 0) && (y1 < wdy) && (x2 >= 0) && (x2 < wdx))
        // gradImg(idn,y1,x2,idc) += gradIn(idn,idy,idx,idc) * (y2 - y) * (x-x1);
         //tensorflow::CudaAtomicAdd(  &gradImg(idn,y1,x2,idc)  ,  (T) (gIn * (y2 - y) * (x-x1) ) );
         atomicAdd(  &gradImg(idn,y1,x2,idc) ,  (T) (gIn * (1.0 - dv) * (du) ) );

      if ((y2 >= 0) && (y2 < wdy) && (x1 >= 0) && (x1 < wdx))
        // gradImg(idn,y2,x1,idc) += gradIn(idn,idy,idx,idc) * (y - y1) * (x2-x);
         //tensorflow::CudaAtomicAdd(  &gradImg(idn,y2,x1,idc)  ,  (T) (gIn * (y - y1) * (x2-x) ) );
         atomicAdd(  &gradImg(idn,y2,x1,idc) ,  (T) (gIn * (dv) * (1.0 - du) ) );

      if ((y2 >= 0) && (y2 < wdy) && (x2 >= 0) && (x2 < wdx))
        // gradImg(idn,y2,x2,idc) += gradIn(idn,idy,idx,idc) * (y - y1) * (x-x1);
         //tensorflow::CudaAtomicAdd(  &gradImg(idn,y2,x2,idc)  ,  (T) (gIn * (y - y1) * (x-x1) ) );
         atomicAdd(  &gradImg(idn,y2,x2,idc) ,  (T) (gIn * (dv) * (du) ) );

      // Gradients wrt. the coordinates  (add because of colour channels)       
      //tensorflow::CudaAtomicAdd( &gradFlow(idn,1, idy, idx) , (T) (gIn * ( (y2- y) * (values_at_top_right-values_at_top_left)  + (y-y1) *  (values_at_bot_right-values_at_bot_left) ) ) );
      //tensorflow::CudaAtomicAdd( &gradFlow(idn,0, idy, idx) , (T) (gIn * (horizontal_interpolated_bot - horizontal_interpolated_top) ) );
      atomicAdd( &gradFlow(idn,0, idy, idx) , (T) (gIn * ( (1.0-dv) * (values_at_top_right-values_at_top_left)  +  dv *  (values_at_bot_right-values_at_bot_left) ) ) );
      atomicAdd( &gradFlow(idn,1, idy, idx) , (T) (gIn * (horizontal_interpolated_bot - horizontal_interpolated_top) ) );
    }
  }
};
...
// Implementing the GPU version of the functor here
template <typename T>
struct WarpimageGradientsFunctor<GPUDevice, T> {
  void operator()(tensorflow::OpKernelContext* context, 
                  const typename Tensor4<T>::ConstTensor  &img ,
                  const typename Tensor4<T>::ConstTensor  &flow ,
                  const typename Tensor4<T>::ConstTensor  &gradIn ,
    ...
    thrust::fill(thrust::device_ptr<T>(gradImg.data()), 
                 thrust::device_ptr<T>(gradImg.data()+gradImg.size()),
                  T(0));
    thrust::fill(thrust::device_ptr<T>(gradFlow.data()), 
                 thrust::device_ptr<T>(gradFlow.data()+gradFlow.size()),
                  T(0));    
    ...
    // WarpimageGPU<T><<<grid_size,block_size>>>(img ,flow, out,  interp_type);
    if (interp_type == tficg::INTERP_TYPE_BILINEAR){
      WarpimageGradientsGPU<T><<<grid_size,block_size,0,d.stream()>>>(img ,flow,gradIn, gradImg, gradFlow, interp_type);
    }
    else if ((interp_type == tficg::INTERP_TYPE_BICUBIC_2POINTS) or (interp_type == tficg::INTERP_TYPE_BICUBIC_4POINTS)){
      printf("\n Gradients are only implemente w.r.t. image not the flow input!\n ");
      ...
      WarpimageGradientsGPUBicubic<T><<<grid_size,block_size,0,d.stream()>>>(img ,flow,gradIn, gradImg, gradFlow, interp_type);
    }
    else
      printf("\n !!!ERROR UNKNOWN FILTER INTERPOLATION TYPE !!!\n ");
...
#define REGISTER_GPU_FUNCTOR(T) \
template struct WarpimageGradientsFunctor<GPUDevice, T>;
// for a full list of type macros see https://github.com/tensorflow/tensorflow/blob/master/tensorflow/core/framework/register_types.h
TF_CALL_GPU_NUMBER_TYPES_NO_HALF(REGISTER_GPU_FUNCTOR);
