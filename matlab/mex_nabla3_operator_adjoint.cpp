...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 1)
        mexErrMsgIdAndTxt(
            "MATLAB:nabla3operatoradjoint:invalidNumInputs",
            "1 input required");
    if (nlhs > 1)
        mexErrMsgIdAndTxt("MATLAB:nabla3operatoradjoint:maxlhs",
                          "Too many output arguments.");

    // parse input volume
    std::unique_ptr<iu::LinearDeviceMemory<T, 4>> iu_in = getLinearDeviceFromMex<T, 4>(*(prhs[0]), false);

    // output memory
    iu::Size<3> out_size({iu_in->size()[0], iu_in->size()[1], iu_in->size()[2]});
